...
| `#tsTable` / `#tsBody` / `#tsTotal` | rows: `<tr><td class="editable practice-td"><div class="practice-badge"><span class="practice-code [unknown]">code</span><span class="practice-name-label">name</span></div></td><td class="editable">ore</td><td class="desc-td"><div class="desc-cell"><span class="desc-text">…</span><button class="desc-edit-btn">icon('edit',12)</button></div></td></tr>`; inline inputs use `class="inline-input"`; clicking the practice cell opens a `<select class="inline-select">` listing every practice (field `pratica`); add `.saving` to the `<tr>` while persisting. `#tsTotal` gets `.total-ok` when `|total − config.daily_hours| < 0.001` else `.total-ko` |
...
| Timesheet | `.elab-empty`, `.elab-empty-icon`, `.elab-result`, `.elab-header-row`, `.elab-updated` (+ `.edited`), `.elab-header-actions`, `.elab-table-wrap`, `.ts-table`, `td.editable`, `td.desc-td`, `.desc-cell`, `.desc-text` (+ `.copied`), `.desc-edit-btn`, `.inline-input`, `.inline-select`, `td.practice-td`, `.practice-badge`, `.practice-code` (+ `.unknown`), `.practice-name-label`, `tr.saving`, `.ts-total-label`, `.total-ok` / `.total-ko`, `.elab-note` |
